...
## Автоматический деплой - что работает, а что нет

### ✅ Что работает автоматически при деплое:

1. **Обновление кода из Git** — при `git push origin dev` или `git push origin master` код обновится автоматически
2. **Пересборка Docker образов** — если изменился `Dockerfile`, образы пересоберутся
3. **Перезапуск контейнеров** — контейнеры перезапустятся с новой конфигурацией
4. **Запуск миграций** — новые миграции из `database/migrations/` выполнятся автоматически
5. **Очистка и пересборка кешей** — кеши конфигурации, роутов, вьюшек очистятся и пересоберутся
6. **Изменения в `docker-compose.yml`** — новые порты, volumes, environment переменные применятся автоматически

### ⚠️ Что нужно делать вручную:

1. **Изменения в `.env` файле**
   - `.env` файлы не отслеживаются в Git (для безопасности)
   - Если изменили переменные окружения (например, `DB_PASSWORD`, `MAIN_DOMAIN`), нужно обновить на сервере вручную
   - **Решение:** Сохраняйте `.env.example` с примерами, или документируйте изменения

2. **Изменения в Nginx конфигурации**
   - Конфиги Nginx (`/etc/nginx/sites-available/*`) не в Git
   - Если добавили новый домен или изменили настройки, нужно обновить вручную
   - **Решение:** Можно хранить конфиги Nginx в Git (например, в `nginx/`) и копировать их при деплое

3. **DNS настройки**
   - DNS записи настраиваются отдельно у провайдера
   - Новые поддомены нужно добавить в DNS вручную

4. **Первый запуск на новом сервере**
   - Настройка Nginx конфигов (см. раздел выше)
   - Создание `.env` файлов (см. раздел выше)
   - Настройка DNS

### Примеры:

✅ **Автоматически работает:**
- Добавили новую миграцию `2025_11_20_create_new_table.php` → выполнится автоматически при деплое
- Изменили код в `app/Models/User.php` → обновится автоматически при деплое
- Изменили `docker-compose.yml` (добавили порт) → применится автоматически
- Изменили `Dockerfile` → образ пересоберется автоматически

⚠️ **Требует ручного вмешательства:**
- Изменили `DB_PASSWORD` в `.env` → нужно обновить на сервере вручную: `nano /var/www/crater/.env`
- Добавили новый домен `test2.crater.billing.mycloud.kg` → нужно настроить Nginx вручную
- Изменили `MAIN_DOMAIN` в `.env` → нужно обновить на сервере и возможно Nginx
...
6. **GitHub Actions workflow** настроен так, что:
   - При пуше в `dev` → деплой на `/var/www/crater-dev`
   - При пуше в `master` → деплой на `/var/www/crater`
   - Все команды выполняются автоматически через SSH
